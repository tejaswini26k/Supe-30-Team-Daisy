...
export default function Stores() {
  ...
  const fetchUserStoreStatus = async () => {
    try {
      ...
      if (!backendStoreId || backendStoreId === "NULL") {
        setIsEditing(true);           // 👈 Allow direct edits
        setStoreSaved(false);
        return;
      }

      setStoreSaved(true);
      setSavedStoreId(backendStoreId);
      setIsEditing(false);            // 👈 Form locked until "Edit" clicked

      const storeRes = await fetch(`http://localhost:5000/api/stores_backup/${backendStoreId}`);
      const { store } = await storeRes.json();

      // ✅ Prefill store details
      ...
      setEmail(store.store_email);
      setPassword(store.password || ""); // 👈 prevent undefined
      if (store.logo) setLogoPreview(store.logo);
      if (store.banner_image) setBannerPreview(store.banner_image);
