...
export default function Stores() {
  ...
  fetch(`http://localhost:5000/api/users/email/${userEmail}`)
    .then(res => res.json())
    .then(user => {
      if (user.store_id) {
        setSavedStoreId(user.store_id);
        setStoreSaved(true);
        setIsEditing(false);

        fetch(`http://localhost:5000/api/stores_backup/${user.store_id}`)
          .then(res => res.json())
          .then(store => {
            ...
            setBusinessType(store.business_type || "Retail");
            setStoreAddress(store.store_address || "");
            setLogoPreview(store.logo_url || null);
            setBannerPreview(store.banner_image_url || null);
          });
      } else {
        setEmail(userEmail);
        setIsEditing(true); 
